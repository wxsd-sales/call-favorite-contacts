...
let sip = null;
let startTime = null;
const serverUrl = 'https://proximal-phantom-muscari.glitch.me/data/';
...
function startListeners() {
  ...
  listeners.push(xapi.Status.SystemUnit.State.NumberOfActiveCalls.on(processCallCount))
    ...
    if (startTime) {
      ...
      if (updatedTime >= 0) {
        ...
        return xapi.Command.HttpClient.Post(
          { AllowInsecureHTTPS: 'True', Header, ResultBody: 'PlainText', Url: serverUrl + fromContact }, JSON.stringify(payload))
          .then(result => {
              ...
              .then(url => {
                ...
                if (fromContact) {
                  if (currentState === 'Standby') {
                    ...
                    const newUrl = prehash + "/form?email=" + fromContact + '#state=' + currentState;
